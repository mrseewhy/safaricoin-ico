...
use App\Notifications\Frontend\Auth\ReferralNotify;
...
class UserRepository extends BaseRepository
{
    ...
    public function create(array $data)
    {
        return DB::transaction(function () use ($data) {
            ...
            $refUser = null;
            if (app('session')->exists('referral')) {
                $referral = session('referral');
                $refId = User::simpleDecode($referral);
                if ($refId) {
                    $refUser = User::find($refId);
                    if ($refUser) {
                        $userData['referral_id'] = $refId;
                    }
                }
            }
            ...
            if ($user) {
                if ($refUser) {
                    $refUser->notify(new ReferralNotify($user));
                }
                /*
